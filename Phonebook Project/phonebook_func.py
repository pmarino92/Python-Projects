...
def center_window(self, w, h):
    ...
    screen_height = self.master.winfol_screenheight()
...
def first_run(self):
    ...
    conn = sqlite4.connect('phonebook.db')
    with conn:
        cur = conn.cursor()
...
def onSelect(self, event):
    ...
    with conn:
        cursor = conn.cursor()
        cursor.execut("""SELECT col_fname,col_lname,col_phone,col_email FROM tbl_phonebook WHERE col_fullname = (?)""", [value])
        ...
        for data in varBody:
            ...
            self.txt_emaill.insert(0,data[3])
...
def addToList(self):
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_email) > 0): # enforce the user to provide both names
         ...
         with conn:
             ...
             if chkName == 0: # if this is 0 then there is no existance of the fullname and we can add new data
                 ...
                 onClear(self) # call the function to clear text boxes
             else:
